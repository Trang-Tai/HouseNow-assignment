    ...
    parent.current &&
      autoAnimate(parent.current, (el, action) => {
        ...
        if (action === 'remove') {
          keyframes = [
            { transform: 'scale(1)', opacity: 1 },
            { transform: 'scale(0.75)', opacity: 1, offset: 0.33 },
            { transform: 'scale(0.5)', opacity: 0.1, offset: 0.5 },
            { transform: 'scale(0)', opacity: 0 },
          ]
        }
  ...
  return (
    <ul className="grid grid-cols-1 gap-y-3" ref={parent}>
      {todos
        ...
        .map((todo) => (
          <li key={todo.id}>
            <div className="flex items-center gap-4 self-stretch rounded-12 border border-gray-200 py-3 pl-4 pr-3 shadow-sm">
              <div className="flex flex-[1_0_0] items-center gap-3">
                <Checkbox.Root
                  id={String(todo.id)}
